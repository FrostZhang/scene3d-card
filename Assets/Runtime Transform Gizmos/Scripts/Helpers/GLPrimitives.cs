    ...
    public static class GLPrimitives
    {
        ...
        public static Matrix4x4 CameraViewMatrix
        {
            get
            {
                ...
                return toRightHanded * Camera.transform.localToWorldMatrix.inverse;
            }
        }
        ...
        public static void DrawWireSelectionBoxes(List<ObjectSelectionBox> selectionBoxes, float boxSizeAdd, Camera camera, Color lineColor, Material boxLineMaterial)
        {
            ...
            for (int selectionBoxIndex = 0; selectionBoxIndex < selectionBoxes.Count; ++selectionBoxIndex)
            {
                ...
                modelSpaceBox.Size = Vector3.Scale(modelSpaceBox.Size, matrixScale) + Vector3.one * boxSizeAdd;
        ...
        public static void DrawGridLines(float cellSizeX, float cellSizeZ, float yPos, Camera camera, Material material, Color color, float camFarPlaneScale)
        {
            CameraViewVolume viewVolume = camera.GetViewVolume(camera.farClipPlane * camFarPlaneScale);
            ...
            Vector3 yOffset = Vector3.up * yPos;
            for(int cell = minCellIndex; cell <= maxCellIndex; ++cell)
            {
                Vector3 xOffset = cell * Vector3.right * cellSizeX + yOffset;
                GL.Vertex(xOffset + Vector3.forward * startZ);
                GL.Vertex(xOffset + Vector3.forward * endZ);

                Vector3 zOffset = cell * Vector3.forward * cellSizeZ + yOffset;
        ...
        public static void DrawWireOOBB(OrientedBox oobb, Color wireColor, Material material)
        {
            GL.PushMatrix();
            GL.LoadIdentity();
            GL.MultMatrix(CameraViewMatrix * oobb.TransformMatrix);
            ...
            {               
                ...
                // Front face
                GL.Vertex(cornerPoints[(int)BoxCornerPoint.FrontTopLeft]);
                GL.Vertex(cornerPoints[(int)BoxCornerPoint.FrontTopRight]);

                GL.Vertex(cornerPoints[(int)BoxCornerPoint.FrontTopRight]);
                GL.Vertex(cornerPoints[(int)BoxCornerPoint.FrontBottomRight]);

                GL.Vertex(cornerPoints[(int)BoxCornerPoint.FrontBottomRight]);
                GL.Vertex(cornerPoints[(int)BoxCornerPoint.FrontBottomLeft]);

                GL.Vertex(cornerPoints[(int)BoxCornerPoint.FrontBottomLeft]);
                GL.Vertex(cornerPoints[(int)BoxCornerPoint.FrontTopLeft]);

                // Back face
                GL.Vertex(cornerPoints[(int)BoxCornerPoint.BackTopLeft]);
                GL.Vertex(cornerPoints[(int)BoxCornerPoint.BackTopRight]);

                GL.Vertex(cornerPoints[(int)BoxCornerPoint.BackTopRight]);
                GL.Vertex(cornerPoints[(int)BoxCornerPoint.BackBottomRight]);

                GL.Vertex(cornerPoints[(int)BoxCornerPoint.BackBottomRight]);
                GL.Vertex(cornerPoints[(int)BoxCornerPoint.BackBottomLeft]);
