...
namespace RTEditor
{
    public class MouseCursor : SingletonBase<MouseCursor>
    {
        ...
        {
            Ray ray;
            if (!InputDevice.Instance.GetPickRay(EditorCamera.Instance.Camera, out ray)) return null;
        ...
        public bool IntersectsPlane(Plane plane, out Vector3 intersectionPoint)
        {
            ...
            Ray ray;
            if (!InputDevice.Instance.GetPickRay(EditorCamera.Instance.Camera, out ray)) return false;
        ...
        {
            Ray ray;
            if (!InputDevice.Instance.GetPickRay(EditorCamera.Instance.Camera, out ray)) return new List<GameObjectRayHit>();
